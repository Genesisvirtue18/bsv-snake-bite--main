...
import SarpathonPhotoGallery from '@/components/SarpathonPhotoGallery'
...
function parseVideoLinks(value = '', mediaLabel = 'Training') {
    return String(value || '')
        .split('\n')
        .map(v => v.trim())
        .filter(Boolean)
        .map((line, index) => {
            ...
            return {
                id: `${index}-${line}`,
                title: hasTitle ? parts[0] : `${mediaLabel} Video ${index + 1}`,
...
function getImageAlbums(card, mediaLabel = 'Training') {
    const albums = Array.isArray(card?.trainingImageAlbums)
    ...
    const normalized = albums
        .map((album, index) => ({
            ...
            title: album.title || `${mediaLabel} Album ${index + 1}`,
    ...
    if (oldImages.length) {
        return [
            {
                id: 'training-images',
                title: `${mediaLabel} Images`,
                description: `${mediaLabel} photos and screenshots.`,
...
export default function TrainingMediaPage({ sarpathon = false }) {
    const filters = sarpathon ? [FILTERS[0], FILTERS[2], FILTERS[1]] : FILTERS
    const mediaLabel = sarpathon ? 'Sarpathon' : 'Training'
    ...
    const trainingCard = (sarpathon
        ? content?.awareness?.items?.find(item => item.id === 'sarpathon' || item.href === '/sarpathon' || item.title === 'Sarpathon')
        : content?.access?.items?.[0]) || {}

    const videos = useMemo(() => {
        // New admin Add Video system
        if (
            Array.isArray(trainingCard.trainingVideoItems) &&
            trainingCard.trainingVideoItems.length
        ) {
            return trainingCard.trainingVideoItems
                .map((video, index) => {
                    const url = video.url || video.link || ''

                    return {
                        id: `${index}-${url}`,
                        title: video.title || `${mediaLabel} Video ${index + 1}`,
                        url,
        ...
        return parseVideoLinks(
            trainingCard.trainingVideos ||
            trainingCard.youtubeUrl ||
            trainingCard.videoUrl ||
            trainingCard.youtubeLink ||
            '', mediaLabel
        )
    }, [trainingCard, mediaLabel])

    const imageAlbums = useMemo(() => getImageAlbums(trainingCard, mediaLabel), [trainingCard, mediaLabel])
    const showVideos = filter === 'all' || filter === 'videos'
    const showImages = filter === 'all' || filter === 'images'

    const pageHeader = content?.pageHeaders?.sarpathon || DEFAULT_CONTENT.pageHeaders.sarpathon
    const title = sarpathon ? (pageHeader.title ?? DEFAULT_CONTENT.pageHeaders.sarpathon.title) : (trainingCard.title || 'Training Modules')
    const description = sarpathon
        ? (pageHeader.description ?? DEFAULT_CONTENT.pageHeaders.sarpathon.description)
        : (trainingCard.desc || trainingCard.description || 'Medical resources, ASV administration protocols, clinical education.')
    ...
    const videosSection = showVideos && (
                    <section className="mb-10">
                        <SectionTitle icon={Play} title="Videos" count={videos.length} color="bg-[#201F8F]" />

                        {!videos.length ? (
                            <EmptyBox text={`No ${mediaLabel.toLowerCase()} videos added yet.`} />
                        ) : (
                                    ...
                                    return (
                                        <Card key={video.id} className="overflow-hidden rounded-2xl border bg-white shadow-sm hover:shadow-xl transition group">
                                            <CardContent className="p-0">
                                                <button
                                                    type="button"
                                                    onClick={() => openTrainingVideo(video)}
                                                ...
                                                <div className="p-5">
                                                    <h3 className="font-display font-bold text-lg text-[#09084f]">
                                                        {video.title || `${mediaLabel} Video ${index + 1}`}
                                                    </h3>
    ...
    const imagesSection = showImages && (
                    <section className="mb-10">
                        ...
                        {!imageAlbums.length ? (
                            <EmptyBox text={`No ${mediaLabel.toLowerCase()} image albums added yet.`} />
                        ) : sarpathon ? (
                            <SarpathonPhotoGallery albums={imageAlbums} />
                        ) : (
                                        ...
                                        >
                                            ...
                                            <div className="p-4">
                                                <div className="font-display font-bold text-[#09084f] text-base line-clamp-2">
                                                    {album.title || `${mediaLabel} Album ${index + 1}`}
                                                </div>
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-white via-[#f8fff9] to-[#eef3ff]">
            ...
            <main className="container mx-auto px-4 pt-5 md:pt-7 pb-12">
                ...
                {sarpathon ? <>{imagesSection}{videosSection}</> : <>{videosSection}{imagesSection}</>}
            ...
            {activeVideo && (
                <Dialog open onOpenChange={() => setActiveVideo(null)}>
                    <DialogContent className="max-w-5xl w-[94vw] p-0 overflow-hidden">
                        ...
                        <div className="flex items-center justify-between pl-4 pr-12 py-4 border-b bg-white">
                ...
                <Dialog open onOpenChange={() => setActiveAlbum(null)}>
                    <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                {activeAlbum.description || `${mediaLabel} images and screenshots`}
                            </DialogDescription>
                        </DialogHeader>
